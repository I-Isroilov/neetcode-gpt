...
class Solution:
    def batch_loader(self, raw_dataset: str, context_length: int, batch_size: int) -> Tuple[List[List[str]], List[List[str]]]:
        ...
        starts = torch.randint(low=0, high=len(tokens) - context_length, size=(batch_size,))

        X = [tokens[i: i + context_length] for i in starts.tolist()]
        Y = [tokens[i + 1: i + 1 + context_length] for i in starts.tolist()]
